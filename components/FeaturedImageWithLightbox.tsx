...
export function FeaturedImageWithLightbox({
  src,
  alt = "",
  caption,
  objectPosition,
  ...
  // Native DOM listeners so zoom always fires regardless of React or overlay
  useEffect(() => {
    if (!open) return;
  ...
  }, [open, close, zoomIn, zoomOut, handlePointerMove, handlePointerUp]);

  return (
    <>
      <figure className="mb-8 group">
        <div
          className="relative aspect-video rounded-lg overflow-hidden ornament-border cursor-pointer transition-[border-color,box-shadow] duration-200 hover:border-alterun-gold/40 hover:shadow-[0_0_20px_-4px_rgba(201,162,39,0.12)]"
        ...
        >
          <Image
            ...
            style={{
              objectPosition: objectPosition === "bottom" ? "bottom" : objectPosition === "center" ? "center" : "top",
            }}
      ...
      {open && mounted && createPortal(
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Featured image full size view"
        >
          <button
            type="button"
            onClick={close}
          ...
          <div
            ref={contentRef}
            className={`relative z-10 max-h-[90vh] w-full max-w-5xl ${isClosing ? "animate-lightbox-content-out" : "animate-lightbox-content"}`}
